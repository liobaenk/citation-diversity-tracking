...
```{r}
own_first = "First_name,Last_name" # e.g. "Kim,Muster"
...
own_year = "publication_year" # "2022", "2030", ...
```
  ...
  annotate(geom="text", 
           ...
           label=paste(as.character(
             round( length(unique(df$CitationKey)) / 
                      (sum(your_summary$your_count) + length(unique(df$CitationKey)) + nrow(n_unknown_pairs)) * 100,2)),
                       "%","\nof all citations","\n(n=",
             as.character(sum(your_summary$your_count) + length(unique(df$CitationKey)) + nrow(n_unknown_pairs)),")", sep=""),
