...
inpath = "manuscript-results/2025_steinfath-et-al_hep-review/OwnCitationBehav/genderapi_cleanbib/" # "data/"
outpath = "manuscript-results/2025_steinfath-et-al_hep-review/output/" # "output/"
...
own_first = "Paul,Steinfath"
own_last = "Arno,Villringer"

# case shared authorship: insert, if applies
#own_first2 = "First_name,Last_name"
own_last2 = "Vadim,Nikulin"
...
```{r}
own_project = "hep-review"
own_journal = "bioRxiv" # bioRxiv, J Neurosci, NeuroImage, PLOS Biolog, ...
own_year = "2025" # "2022", "2030", ...
```
...
```{r}
predict_name = "predictions_v2"
cbib_name = "cleanedBib_v2"

predictions = read.csv(paste(inpath,predict_name,".csv",sep=""))
cleanedbib = read.csv(paste(inpath,cbib_name,".csv",sep=""))

if (review == 1) {
  predictions_ReviewLit = read.csv("manuscript-results/2025_steinfath-et-al_hep-review/ReviewLit/genderapi_cleanbib/predictions.csv")
}
...
if (review == 1) {
  ...
  analyse_review_or_ownciting = "ownciting" # review
...
check_predictions$Gender_posthoc = c(
  ...
  "female", "male", "female", "male", "female", "unknown", "unknown", "male", "male", "unknown",
...
This is done to distinguish between (1) references from the reviewed literature and (2) additional references cited in the review's main text.
...
if (review == 1) {
  ...
  # percent
  percent_reviewed_cited = round(n_reviewed_cited / n_reviewed * 100,4)
...
(1) References from the reviewed literature (ReviewLit) that are cited in the main text:

```{r, echo=FALSE, layout="l-body-outset"}

if (exists("predictions_own_ReviewLit")) {
  paged_table(predictions_own_ReviewLit[,c("X","CitationKey","Author","GendCat_posthoc")])
} else {
  cat("No file displayed, as review == 0.")
}

```

(2) Additional references cited in the main text:

```{r, echo=FALSE, layout="l-body-outset"}

if (exists("predictions_own_new")) {
  paged_table(predictions_own_new[,c("X","CitationKey","Author","GendCat_posthoc")])
} else {
  cat("No file displayed, as review == 0.")
}
...
if (review == 0) {
  ...
  group_by(GendCat_posthoc) %>%
  summarise(your_count = n(),
            your_percent = n() / nrow(df_plot[(df_plot$GendCat != ""),]) * 100)
  }
